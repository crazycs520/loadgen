...
sysbench.cmdline.options = {
   ...
   simple_join =
      {"Number of simple join", 1},
   simple_aggs =
      {"Number of simple aggs", 1},
   join_aggs =
      {"Number of join aggs", 1},
   simple_ranges =
...
CREATE TABLE sbtest%d(
  ...
  %s (id, k)
      ...
      table_num, id_index_def, engine_def, extra_table_options)
...
local stmt_defs = {
   ...
   batch_point_selects = {
      ...
      {t.VARCHAR, 64}, {t.VARCHAR, 64}, {t.VARCHAR, 64}, {t.VARCHAR, 64}, {t.VARCHAR, 64},
      {t.VARCHAR, 64}, {t.VARCHAR, 64}, {t.VARCHAR, 64}, {t.VARCHAR, 64}, {t.VARCHAR, 64}},
   index_scan_point = {
   ...
   stale_read_index_scan_ranges = {
      ...
      t.INT, t.INT},
   simple_join = {
      "SELECT /*+ TIDB_INLJ(t1, t2) */ t2.k FROM sbtest%u t1 join sbtest%u t2 USING(id) where t1.id between ? AND ?",
      t.INT, t.INT},
   simple_aggs = {
      "SELECT length(group_concat(c)) FROM sbtest%u WHERE id BETWEEN ? AND ?",
      t.INT, t.INT},
   join_aggs = {
      "SELECT /*+ TIDB_INLJ(t1, t2) */ length(group_concat(t2.c)) FROM sbtest%u t1 join sbtest%u t2 USING(id) where t1.id between ? AND ?",
      t.INT, t.INT},
   simple_ranges = {
...
function prepare_for_each_table2(key)
...
function prepare_simple_join()
   prepare_for_each_table2("simple_join")
end


function prepare_join_aggs()
   prepare_for_each_table2("join_aggs")
end

function prepare_simple_aggs()
   prepare_for_each_table("simple_aggs")
end
...
function execute_batch_point_selects()
   ...
   for i = 1, sysbench.opt.batch_point_selects do
      ...
      param[tnum].batch_point_selects[1]:set(get_string_id())
      param[tnum].batch_point_selects[2]:set(get_string_id())
      param[tnum].batch_point_selects[3]:set(get_string_id())
      param[tnum].batch_point_selects[4]:set(get_string_id())
      param[tnum].batch_point_selects[5]:set(get_string_id())
      param[tnum].batch_point_selects[6]:set(get_string_id())
      param[tnum].batch_point_selects[7]:set(get_string_id())
      param[tnum].batch_point_selects[8]:set(get_string_id())
      param[tnum].batch_point_selects[9]:set(get_string_id())
      param[tnum].batch_point_selects[10]:set(get_string_id())
...
function execute_simple_aggs()
   execute_range("simple_aggs")
end

function execute_join_aggs()
    execute_range("join_aggs")
end

function execute_simple_join()
    execute_range("simple_join")
end
